...
const csvFilename = 'school-leaver-destinations.csv';
...
function interrogateSPARQL(zones) {
    ...
    _.forEach(_.chunk(zones, chunk_size), (zone_chunk) => {
        queries.push(`
            ...
            SELECT ?year ?zone ?group ?gender ?destination ?measure ?value
            WHERE {
                ?s qb:dataSet data:school-leaver-destinations ;
                   qb:measureType ?m ;
                   ?m ?value ;
                   dim:gender ?g ;
                   dim:schoolLeaverDestination ?d ;
